...
```{r, echo = FALSE, fig.fullwidth = TRUE, message = FALSE, results = 'hide'}
by_date = foo %>% count(dt, severity, .drop = FALSE) %>% mutate(`Day of Week` = weekdays(dt), is_weekend = ifelse(weekdays(dt) %in% c('Saturday', 'Sunday'), 'weekend', 'weekday'))
...
6. Upon what date did the highest total number of accidents occur?  Examine the data and columns provided, and see if you can determine a cause for the date with the highest total number of accidents.  In general, what a possible explanation for the pattern observed between warm-season (May-Oct) and cold-season (Nov-Apr) Total and Fatal accidents?
