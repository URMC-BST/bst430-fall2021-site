...
The data are made available [online](https://data.gov.uk/dataset/cb7ae6f0-4be6-4935-9277-47e5ce24a11f/road-safety-data/datafile/36f1658e-b709-47e7-9f56-cca7aefeb8fe/preview) by the NY government.
...
-   `emo::ji("broom")` Run `lint_assignment()` and verify that you are lint free.
...
```{r inline-nrows, include = F}
nrows <- nrow(crashes)
```

The data set contains `r nrows` rows.
...
On weekdays, fatal accidents seem to increase towards the afternoon, peaking around rush hour. During the weekends, fatal accidents are more steady, peaking slightly around 2AM and the afternoon. Property accidents on the other hand appear to have a bimodal distribution on weekdays, peaking around 9AM and 5PM. On weekends, there is a milder peak around 2PM. 

`emo::ji("broom")` 🧶 ✅ ⬆️ Lint, Knit, *commit, and push your changes to GitHub with an appropriate commit message. Make sure to commit and push all changed files so that your Git pane is cleared up afterwards.*
...
```{r finding most accidents, include=F}
worst.date<-date_severity$dt[which.max(date_severity$Total)]
mean(crashes$`Weather Conditions`[crashes$dt == worst.date] == "Snow")
```

The date with the highest total number of accidents was `r worst.date`. This could have been due to heavy snow on this date, as over half the crashes occurred under this condition. One possible explanation to the pattern is that, in the cold season, weather increases the number of accidents, but these are typically lower speed collisions, resulting in fewer fatalities. In the summer, there are fewer weather conditions to cause accidents, but the ability to drive at higher speeds increases the proportion of fatal accidents.
...
```{r one-example}
by_date_w = crashes  %>% count(dt, `Weather Conditions`, .drop = FALSE)
date_severity_weather = by_date_w %>% group_by(dt) %>% mutate(Total = sum(n)) %>%
  pivot_wider(c(dt, Total), names_from = `Weather Conditions`, values_from = n) %>%
  mutate(m = month(dt), season = case_when(
    m >= 11 | m <= 4 ~ 'cold',
    TRUE ~ 'warm'))

ggplot(date_severity_weather, aes(x = Total, y = Clear, color = season)) + geom_jitter(height = .2) + 
  labs(y = 'N. Clear', x = 'N. Total',   
       title = "Daily Clear Weather Accidents vs Daily Total Accidents",
    subtitle = "New York, 2018-2019") + 
  theme_minimal() + scale_color_manual(labels = c("Nov-Apr", "May-Oct"), values = c('blue', 'red'))
