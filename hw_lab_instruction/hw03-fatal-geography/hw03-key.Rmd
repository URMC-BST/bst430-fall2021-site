...
```{r}
crash_count = crashes %>% mutate(across(c(county_name, is_pedbike), as.factor)) %>% count(county_name, is_pedbike, .drop = FALSE) %>% filter( county_name != 'Unknown') %>% replace_na(list(n = 0))
...
fct_lump_reorder = function(factor_var, n_var){
   fct_lump_n(fct_reorder({{factor_var}}, {{n_var}}), 20, w = {{n_var}})
}
...
top_20_popcrash = crash_count %>% group_by(county_name) %>% summarize(total_per_100k = sum(n_per_100k)) %>% 
  ungroup() %>% mutate(is_top20 = ifelse(min_rank(-total_per_100k)<=20, county_name, "Rest of NY")) 

crash_100k = crash_count %>% left_join(top_20_popcrash %>% select(-total_per_100k), by = 'county_name') %>% 
  group_by(is_top20, is_pedbike) %>% summarize(n_per_100k = mean(n_per_100k))

(plt %+% crash_100k) + aes(y = fct_reorder(is_top20, n_per_100k), x = n_per_100k)
```
...
adjusted_wide = adjusted %>% select(-n_per_100k, -n) %>% pivot_wider(names_from = is_pedbike, values_from = n_per_100k_vmt)
...
The fatalities per 100k VMT is most informative for motorists about their marginal risk of driving.  For pedestrians/cyclists, we really would like to know how many miles pedestrians/cyclists logged in a county in order to adjust for the amount of time they spend on the roads.  This would likely make NYC environs appears less hazardous.
