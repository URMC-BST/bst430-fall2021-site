...
Again, we use the **tidyverse**, `vroom` and `lubridate` packages.
...
```{marginfigure}
**Hint**: your life will be made easier if you construct a "crosswalk" mapping the identifiers between the VMT dataset `urbanized_area` and `county_name` from the crashes data, either as a .csv file that you read in with `read_csv` or using the `tibble` or `tribble` function directly in your markdown. Here's an example of the first seven rows of such a crosswalk:
![](img/vmt-crosswalk.png)
```

7.  Find the 13 rows in the VMT data that are primarily in New York (i.e. ignoring Dansbury and Bridgeport, CT-NY, which honestly do not include much, if any of New York) and contain a non-missing value for `vehicle_miles_traveled_per_capita_raw_value` using `filter` and `str_detect`. Using whatever resources you wish--google works, though it's a bit tedious--to identify the counties corresponding to these `urbanized_areas`, and join this to the table.  (This will not be a one-to-one join.)  Then join the fixed up VMT table to the fatal crash counts.


7.  Derive the fatalities per 100,000 vehicle miles traveled, per county, using the crash count table Ex 3 and 5.  Repeat your plot from Ex 5 (though now you will only have 16 counties).  Discuss your findings.
