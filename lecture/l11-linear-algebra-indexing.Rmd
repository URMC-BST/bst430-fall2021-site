...
Let's suppose [we tabulate](#tabulate-flights) the average delay at the NYC airports at every hour during the month of June.
...
airport_delay_plot + labs(title = "Total delay (minutes) on each hour")
```
...
.pull-left[
```{r, echo = FALSE, ref.label="delay-cow",  warning=FALSE, message=FALSE, fig.asp = 1, fig.width = 6}
```
]
.pull-right[
...
knitr::include_graphics('l11-WIP/img/major-order.svg')
```
...
<a name = "tabulate-flights">

```{r, ref.label='tabulate-flights', echo = TRUE, eval = FALSE}
```
