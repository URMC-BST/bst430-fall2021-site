...
```{r, output.lines = 13}
diagnoses = read_csv('l10-WIP/data/cardiac-dx.csv')
diagnoses
```
...
diagnoses_row %>% filter(str_detect(diagnoses, 'hypoplastic')) %>%
  ...
  labs(y = "Diagnosis", x = 'Patient', 
       main = 'Co-occurance of hypoplastic heart disorders')
```
...
```{r}
ggplot(word_sentiments, aes(y = fct_reorder(word_collapse,  n, .fun = sum), x = n, fill = sentiment)) + geom_col() + facet_wrap(~title, scales = 'free_x') + ylab("Word") + xlab("Occurence") + theme_minimal()
```
...
The term frequency is just the word counts, normalized to the number of words per text, so the popular TF-IDF is just
